...
//				byte[] to_send = toByteArray2(writeText.getText().toString());
				...
				byte[] writeBuffer = {0x55,(byte)0xaa,0x01,0x05,0x05};
				int retval = MyApp.driver.WriteData(writeBuffer, writeBuffer.length);//写数据，第一个参数为需要发送的字节数组，第二个参数为需要发送的字节长度，返回实际发送的字节长度
				Log.i(TAG,"driver,writeBuffer 0x55,(byte)0xaa,0x01,0x05,0x05");
				if (retval < 0){
					Toast.makeText(MainActivity.this, "写失败!", Toast.LENGTH_SHORT).show();
					Log.i(TAG,"driver,writeBuffer failed");
				}
	...
	private void initUI() {
		...
		clearButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				Log.i(TAG, "clearButton,onClick,ReadData start");

				//read height request
				byte[] buffer = new byte[6];
				int length = MyApp.driver.ReadData(buffer, buffer.length);
				Log.i(TAG, "readCH34xUARTDriver,ReadData,length=" + length);
				if( length > 0 && buffer[0] == 0x55 && buffer[1] == 0xAA ){
					short height = (short) (((buffer[4] & 0x000000FF) << 8) | (0x000000FF & buffer[5]));
					float mHeight = height / 10f;
					Log.i(TAG, "readCH34xUARTDriver,ReadData,mHeight=" + mHeight);
				}
			}
		});
		return;
